...
Nel tronco convergente, l'area decresce. L'algoritmo calcola il termine sorgente di spinta geometrica $p \frac{dA}{dx}$ (che è negativo, opponendosi al flusso). Il flusso accelera fino a raggiungere esattamente Mach 1 ($u = a$) in corrispondenza del diametro minimo ($D=0.1$ m, a $x=0.5$ m). La portata massica si fissa al suo valore di \textit{choking}. Nel codice, la stabilità in questa delicata fase transonica è garantita dal Correttore di Entropia di Harten (Eq. 24).

\subsubsection{Fase 2: Regime Supersonico}
Superata la gola, il condotto diventa divergente. Essendo il flusso già sonico, l'espansione geometrica forza un'ulteriore accelerazione. Il flusso diventa \textbf{supersonico} (Mach $> 1$) e la pressione statica $p$ crolla rapidamente, scendendo abbondantemente sotto i $400\,000$ Pa. 

\subsubsection{Fase 3: Formazione dell'Urto Normale}
Essendo la pressione ambiente esterna $P_{\text{amb}} = 400\,000$ Pa molto più alta della pressione statica generata dall'espansione supersonica, l'informazione di alta pressione cerca di risalire la corrente. Tuttavia, siccome il fluido è supersonico, le onde acustiche non possono tornare indietro. 
Le onde di compressione si accumulano in un punto specifico del divergente generando una \textbf{discontinuità stazionaria}: un Urto Normale.

Il Risolutore di Riemann di Roe rileva che tra due celle adiacenti (ad esempio $i$ e $i+1$) esiste un enorme $\Delta p$ in contrapposizione a un flusso supersonico. Calcolando gli autovalori $\lambda_k$, il termine di dissipazione upwind frena istantaneamente il gas. 

Attraverso la "faccia" dell'urto, il solver preserva rigorosamente l'uguaglianza dei flussi conservativi $\mathbf{F}(\mathbf{U}_L) = \mathbf{F}(\mathbf{U}_R)$:
\begin{itemize}
    \item $U_0$ (massa): La densità $\rho$ aumenta repentinamente, compensando l'esatto calo di velocità $u$ in modo che $\rho u$ resti costante.
    \item $U_1$ (quantità di moto): L'imponente perdita di velocità causa un salto violento della pressione statica $p$ per preservare l'equilibrio $\rho u^2 + p$.
    \item Il flusso passa istantaneamente da Supersonico (es. Mach 1.8) a Subsonico (es. Mach 0.6).
\end{itemize}

\subsubsection{Fase 4: Riadattamento Subsonico e Uscita}
A valle dell'urto, il gas è tornato subsonico. Poiché siamo ancora nel tratto divergente, e il gas è ora subsonico, l'aumento di sezione provoca un \textbf{rallentamento} della velocità (diffusione subsonica) e un ulteriore recupero graduale di pressione.
Nel tratto finale a tubo dritto, il gas subsonico (in assenza di attrito) mantiene pressione costante, che combacia esattamente, all'ultima cella, con la boundary condition imposta: $p_{N_x-1} \approx 400\,000$ Pa. Il transitorio è concluso e la simulazione ha raggiunto lo Steady-State.
